...
# Remove any existing cleanup cron job to prevent duplicates
(sudo crontab -l | grep -v 'find $MEDIA_PATH' || true) | sudo crontab -
if [ "$CLEANUP_CHOICE" == "1" ]; then
    CLEANUP_SCHEDULE="0 2 * */1 *" # Every 1 month at 2am
    echo "Cleanup scheduled for every 30 days."
elif [ "$CLEANUP_CHOICE" == "2" ]; then
    CLEANUP_SCHEDULE="0 2 * */2 *" # Every 2 months at 2am
    echo "Cleanup scheduled for every 60 days."
elif [ "$CLEANUP_CHOICE" == "3" ]; then
    CLEANUP_SCHEDULE="0 2 * */3 *" # Every 3 months at 2am
    echo "Cleanup scheduled for every 90 days."
fi

if [ "$CLEANUP_CHOICE" != "4" ]; then
    # The find command will remove all files and directories in the media path, excluding the path itself
